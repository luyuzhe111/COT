...
class HistogramDensity: 
    ...
    def fit(self, vals): 
        ...
        self.hist, bin_edges = torch.histogram(vals.double().cpu(), bins=torch.from_numpy(self.bins), density=True)
    
    def density(self, x): 
        curr_bins = np.digitize(x.cpu(), self.bins, right=True)
...
def get_im_estimate(probs_source, probs_target, corr_source): 
	...
	prod = weights*(corr_source.unsqueeze(1).cpu())
	return 1-torch.mean(prod)
