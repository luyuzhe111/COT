...
def gather_outputs(model, dataloader, pooling, layer, device, cache_dir):
        ...
        with torch.no_grad():
            for inputs, targets in tqdm(dataloader):
                ...
                if layer == 'fc':
                    ...
                    act = model.retained_layer(layer).squeeze().flatten(start_dim=2)

                    if pooling == 'max':
                        act = torch.max(act, 2)[0]
                    elif pooling == 'avg':
                        act = torch.mean(act, 2)
                    else:
